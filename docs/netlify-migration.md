...
6. Configurar en la interfaz privada de Netlify, con alcance de Functions, `ADMIN_EMAIL`, `ADMIN_PASSWORD_HASH`, `JWT_SECRET` y, si se usa chat, `OPENAI_API_KEY`. No guardar ni compartir esos valores en el repositorio. `VITE_API_URL` debe quedar sin definir para usar `/api/*` en el mismo origen.
